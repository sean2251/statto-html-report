...
function buildTimeSeriesSection() {
  ...
  function draw(series, isPct, stat) {
    ...
    const DOTS_GAP = 13, TOURN_ROW = hasTournLabels ? 15 : 0;
    const mB = DOTS_GAP + nameBand + TOURN_ROW + 6;
    ...
    // X axis: a win/loss dot per game with the opponent's name tilted below it,
    // the name coloured green for a win and red for a loss.
    const dotsY = mT + plotH + DOTS_GAP;
    const namesTopY = dotsY + 6;
    games.forEach((g, i) => {
      const x = xFor(i);
      const cls = g.result === 'W' ? 'ts-win' : g.result === 'L' ? 'ts-loss' : 'ts-tie';
      svg.appendChild(svgEl('circle', { cx: x, cy: dotsY, r: 3.5, class: cls }));
      const nm = svgEl('text', {
