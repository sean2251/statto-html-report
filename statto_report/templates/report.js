...
// A .game-filter-panel is CSS-anchored to its button's right edge (right: 0),
// so when the button sits near the right of a phone screen the panel can spill
// off the LEFT edge and clip its options. After opening, if the panel's left
// edge is off-screen, re-anchor it from the left so it stays fully visible.
function clampFilterPanel(panel) {
  panel.style.left = '';
  panel.style.right = '';
  const margin = 8;
  const r = panel.getBoundingClientRect();
  if (r.left < margin) {
    const wrapRect = (panel.offsetParent || document.body).getBoundingClientRect();
    panel.style.right = 'auto';
    panel.style.left = (margin - wrapRect.left) + 'px';
  }
}
...
function buildImpactCategoryDropdown(onChange, categoryList) {
  ...
  btn.addEventListener('click', (e) => {
    ...
    if (opening) clampFilterPanel(panel);
  });
  document.addEventListener('click', (e) => {
...
// Line Analysis scoring efficiency, built to compare across lines at a glance:
// one bar per line within each metric (Total/Offensive/Defensive), grouped so
// you can read "which line holds best" straight down a group; a shared mode
// toggle (Per Point / Per Possession / First Possession) redraws all three.
// Reuses the .cmp-* comparison-bar styles.
function buildLineScoringEfficiencyComparison(entities) {
  const wrap = el('div', { class: 'line-eff-compare' }, []);
  const legend = el('div', { class: 'cmp-legend' }, []);
  entities.forEach((en, i) => legend.appendChild(el('span', { class: 'cmp-key' }, [
    el('span', { class: 'cmp-dot', style: 'background:' + compareColorVar(i) }, []),
    document.createTextNode(en.name),
  ])));
  ...
  function render(mode) {
    groups.innerHTML = '';
    SCORING_EFF_GAUGES.forEach(g => {
      const block = el('div', { class: 'cmp-row' }, [el('div', { class: 'cmp-lbl' }, [el('span', {}, [document.createTextNode(g.label)])])]);
      const vals = entities.map(en => (en.scoringEfficiency[mode][g.key] || {}).pct);
      const present = vals.filter(v => v != null);
      const best = present.length > 1 && !present.every(v => v === present[0]) ? Math.max.apply(null, present) : null;
      entities.forEach((en, i) => {
        const d = en.scoringEfficiency[mode][g.key] || {};
        const pct = d.pct;
        const w = pct == null ? 0 : Math.max(2, Math.min(100, pct));
        const isLead = pct != null && best != null && pct === best;
        block.appendChild(el('div', { class: 'cmp-bar' + (isLead ? ' lead' : '') }, [
          el('span', { class: 'cmp-nm' }, [document.createTextNode(en.name)]),
          el('span', { class: 'cmp-track' }, [el('span', { class: 'cmp-fill' + (pct == null ? ' empty' : ''), style: 'width:' + w + '%;' + (pct == null ? '' : 'background:' + compareColorVar(i)) }, [])]),
          el('span', { class: 'cmp-v' }, [document.createTextNode(pct == null ? '–' : Math.round(pct) + '%')]),
        ]));
      });
  ...
  render('perPoint');
  wrap.appendChild(el('div', { class: 'eff-toggle-row' }, [buildSegToggle(SCORING_EFF_MODES, render)]));
  return wrap;
}
...
function buildPlayerSelector(onChange, options) {
  ...
  // One-click "Deselect all", available in every player/line picker.
  const clearBtn = el('button', { class: 'game-filter-clear', type: 'button' }, [document.createTextNode('Deselect all')]);
  clearBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    if (!selectedOrder.length) return;
    selectedOrder = [];
    entries.forEach(({ cb }) => { cb.checked = false; });
    if (selectAllCb) { selectAllCb.checked = false; selectAllCb.indeterminate = false; }
    updateLabel();
    updateDisabledStates();
    onChange(selectedOrder.slice());
  });
  panel.appendChild(el('div', { class: 'game-filter-clearrow' }, [clearBtn]));
...
function buildDistributableHtml(opts) {
  ...
  // per-opponent tagging page) don't all read the same. The parent report's
  // title is baked into the pristine HTML, so rewrite the whole <title> tag.
  if (opts.title) {
    const safe = String(opts.title).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    html = html.replace(/<title>[\s\S]*?<\/title>/i, '<title>' + safe + '<\/title>');
  }
  html = html.indexOf(marker) !== -1
...
function downloadGameTaggingPage(gameIndex) {
  ...
  // Opponent first so it survives tab-title truncation -- this is the one thing
  // that tells a helper's tabs apart when they're tagging several games.
  const title = 'vs ' + g.opponent + ' — ' + REPORT.teamName + ' tagging';
  downloadFile(buildDistributableHtml({ tagOnlyGame: gameIndex, title: title }),
...
function buildLineAnalysisSection(variant) {
  ...
  function buildScopeManagementBlock(scope) {
    ...
    // The point-by-point line builder is authoring UI: the read-only team
    // report drops it entirely -- whoever runs the main report sets the lines,
    // and a matrix of checkboxes just confuses a teammate who only wants to
    // compare the lines that already exist.
    if (!VIEWER_MODE) {
  ...
  function renderCompare() {
    ...
    function renderContent() {
      ...
      contentArea.appendChild(buildLineScoringEfficiencyComparison(entities));
...
const QUERY_KINDS = {
  ...
  // Points is a synthesized kind: rather than filtering tagged events, it lists
  // the start timestamp of each offensive/defensive possession in points the
  // selected player was on the field for (see gatherPoints). No tag filters.
  points: { label: 'Points', word: 'possession starts', filterKeys: [] },
};
...
function buildAnnotationQueryPane() {
  ...
  function buildFilterControls() {
    ...
    if (showKind === 'pass') {
      ...
      // so they re-scope the denominator: "of Jordan's turnovers, how many were
      // around backhands".
      filterRow.appendChild(dropdown('Outcome', ['Completed', 'Turnover', 'Assist'], 'outcome'));
      filterRow.appendChild(dropdown('Thrower', allPlayerNames(), 'thrower'));
      filterRow.appendChild(dropdown('Receiver', allPlayerNames(), 'receiver'));
      ...
      filterRow.appendChild(dropdown('Player', allPlayerNames(), 'player'));
    ...
    } else if (showKind === 'points') {
      filterRow.appendChild(dropdown('Player', allPlayerNames(), 'player'));
      filterRow.appendChild(dropdown('Possession', ['Offensive', 'Defensive'], 'possession'));
    }
  }
  ...
  function describe(ev, a) {
    if (ev.kind === 'pass') {
      ...
      return {
        ...
        turnover: p.turnover, assist: p.assist, thrower: p.thrower, receiver: p.receiver,
      };
    }
    if (ev.kind === 'block') return { main: `${ev.item.player || '?'} — block`, outcome: '', player: ev.item.player };
    if (ev.kind === 'pull') {
      return {
  ...
  // "Points" kind: for every point the selected player was on the field for
  // (any point, whichever side it started on), the start timestamp of each
  // offensive possession (its first tagged pass) and each defensive possession
  // (the tagged #d event, or the pull for the first one). Filtered by player +
  // which side. Rows share the shape gather() returns so appendResultRow reuses.
  function gatherPoints() {
    const ann = loadAnnotations();
    const setup = loadSetupData();
    const rows = [];
    const wantPlayer = filters.player || '';
    const side = filters.possession || ''; // '', 'Offensive', 'Defensive'
    selectedGames.forEach(gi => {
      const g = REPORT.games[gi];
      const vid = parseYouTubeId(setup.videoLinks[gi]);
      (g.points || []).forEach(pt => {
        if (wantPlayer && !(pt.lineup || []).some(e => e.player === wantPlayer)) return;
        const pointRows = [];
        if (side !== 'Defensive') {
          const byPoss = new Map();
          (pt.passes || []).forEach(p => {
          ...
          [...byPoss.keys()].sort((a, b) => a - b).forEach((num, i) => {
            let ts = null;
            byPoss.get(num).forEach(p => { const t = (ann.passes[p.uuid] || {}).timestamp; if (t != null && (ts == null || t < ts)) ts = t; });
            pointRows.push({ side: 'Offensive', idx: i + 1, timestamp: ts });
          });
        }
        if (side !== 'Offensive') {
          const pull = (ann.points || {})[pt.uuid + '#pull'] || {};
          for (let d = 1; d <= (pt.defensivePossessions || 0); d++) {
            let ts = ((ann.points || {})[pt.uuid + '#d' + d] || {}).timestamp;
            if (ts == null && d === 1 && !pt.isOffense) ts = pull.timestamp; // opponent starts on our pull
            pointRows.push({ side: 'Defensive', idx: d, timestamp: ts != null ? ts : null });
          }
        }
        const multiO = pointRows.filter(r => r.side === 'Offensive').length > 1;
        const multiD = pointRows.filter(r => r.side === 'Defensive').length > 1;
        // Video order within the point; untagged possessions sink to the end.
        pointRows.sort((a, b) => (a.timestamp == null ? Infinity : a.timestamp) - (b.timestamp == null ? Infinity : b.timestamp));
        pointRows.forEach(r => {
          const numbered = r.side === 'Offensive' ? multiO : multiD;
          rows.push({
            opponent: g.opponent, point: pt.number,
            main: r.side + ' possession' + (numbered ? ' ' + r.idx : ''),
            outcome: '', a: {}, vid, timestamp: r.timestamp,
          });
        });
      });
    });
  ...
  // Non-tag "data" filters (read off the event/possession itself, not an
  // annotation): they re-scope the population per kind.
  function dataMatch(r) {
    if (showKind === 'pass') {
      return outcomeMatch(r, filters.outcome)
        && (!filters.thrower || r.thrower === filters.thrower)
        && (!filters.receiver || r.receiver === filters.receiver);
    }
    if (showKind === 'block') return !filters.player || r.player === filters.player;
    return true;
  }
  ...
  // Renders one result clip -- shared by the tagged-event kinds and by Points.
  function appendResultRow(r) {
    // Every stored tag value worth showing, minus the ones already spelled out
    // in the row's own label. (Empty for Points rows, which carry no tags.)
    const tags = [];
  ...
  function renderPoints() {
    // Only possessions with a start timestamp are clips; the rest can't be
    // deep-linked, so they're left out rather than listed as "no timestamp".
    const clips = gatherPoints().filter(r => r.timestamp != null);
    const noun = 'possession start' + (clips.length === 1 ? '' : 's');
    const sideWord = filters.possession ? filters.possession.toLowerCase() + ' ' + noun : noun;
    summary.textContent = `${clips.length} ${sideWord} in scope`;
    results.innerHTML = '';
    lastLinks = [];
    if (!clips.length) {
      results.appendChild(el('p', { class: 'pitch-caption' }, [document.createTextNode('No possession-start timestamps in scope — tag the start of possessions in the Tag view, or widen the filters.')]));
      copyBtn.disabled = true;
      return;
    }
    clips.forEach(appendResultRow);
    copyBtn.disabled = lastLinks.length === 0;
  }

  function render() {
    if (showKind === 'points') { renderPoints(); return; }
    const all = gather();
    const population = all.filter(dataMatch);
    ...
    // uses the full data-filtered population as its denominator.
    ...
    matches.forEach(appendResultRow);
    copyBtn.disabled = lastLinks.length === 0;
  }
...
function buildDataEditorSection(viewer, tagOnlyGame) {
  ...
  if (viewer) {
    ...
    section.appendChild(el('p', { class: 'hero-sub' }, [document.createTextNode('Browse the tagged film: filter passes, blocks, pulls, defensive possessions or opponent turnovers — or the start of a player’s possessions — and jump straight to the moment in the game video.')]));
...
// ---------- Time Series section ----------
// One value per game, plotted for the chosen entities. Two modes:
//   * By Player -- per-player box-score and EDGE stats, with optional whole-team
//     average / total lines.
//   * By Line   -- per-line stats (from Line Analysis) plus "All Lines" (the
//     whole team). No average/total there -- All Lines already is the aggregate.
// Stat kinds: 'count' (sum-able -> team total available), 'rate' (a 0-100 %),
// 'rating' (an unbounded score, e.g. EDGE / PE -- mean only, auto-scaled y).
// source 'adv' pulls a game's row from computeAdvancedStats([gi]) instead of the
// box score. `dec` sets display decimals.

// Categorical colours for player/line series -- deliberately free of green and
// red so a plotted line is never confused with the win/loss colouring on the
// x-axis (opponent names + dots).
const TS_ENTITY_COLORS = ['#4E9BE0', '#9B79EA', '#E0952E', '#D061C9', '#5D6FE0', '#B5793F', '#C9A227'];

function tsLineName(line, tournaments) {
  if (!line.tournamentId) return line.name;
  const t = tournaments.find(tt => tt.id === line.tournamentId);
  return line.name + (t ? ' · ' + t.label : '');
}
...
const TIMESERIES_STATS = [
  ...
  // EDGE / advanced, computed per game. EDGE-O/B/EDGE are goal-equivalents (a
  // team total is meaningful -- EDGE-O sums to the team's goals); xE/PE are
  // per-opportunity ratings (mean only). CR is a within-selection percentile, so
  // it has no standalone per-game meaning and is deliberately left out.
  { key: 'edgeO', label: 'EDGE-O', kind: 'count', source: 'adv', dec: 1, get: r => r.edgeO },
  { key: 'edgeB', label: 'EDGE-B', kind: 'count', source: 'adv', dec: 1, get: r => r.edgeB },
  { key: 'edge', label: 'EDGE', kind: 'count', source: 'adv', dec: 1, get: r => r.edge },
  { key: 'xEO', label: 'xEO', kind: 'rating', source: 'adv', dec: 1, get: r => r.xEO },
  { key: 'xEB', label: 'xEB', kind: 'rating', source: 'adv', dec: 1, get: r => r.xEB },
  { key: 'xE', label: 'xE', kind: 'rating', source: 'adv', dec: 1, get: r => r.xE },
  { key: 'cpPlus', label: 'CP+ (%)', kind: 'rate', source: 'adv', get: r => r.cpPlus },
  { key: 'peO', label: 'PE-O', kind: 'rating', source: 'adv', dec: 2, get: r => r.peO },
  { key: 'peB', label: 'PE-B', kind: 'rating', source: 'adv', dec: 2, get: r => r.peB },
  { key: 'pe', label: 'PE', kind: 'rating', source: 'adv', dec: 2, get: r => r.pe },
];
TIMESERIES_STATS.forEach(s => { s.group = s.source === 'adv' ? 'Advanced (EDGE)' : (s.kind === 'rate' ? 'Rates (%)' : 'Counting stats'); });

// By-Line stats -- one value per game, read off computeLineStats() for a line's
// points in that game. Only stats that read cleanly game-by-game are included.
const LINE_TIMESERIES_STATS = [
  { key: 'pointsPlayed', label: 'Points played', kind: 'count', get: l => l.pointsPlayed },
  { key: 'offensePlayed', label: 'Offensive points', kind: 'count', get: l => l.offensePlayed },
  { key: 'defensePlayed', label: 'Defensive points', kind: 'count', get: l => l.defensePlayed },
...
function buildTimeSeriesSection() {
  ...
  section.appendChild(el('p', { class: 'hero-sub' }, [document.createTextNode('Track a stat game by game across the season — by player or by line. Add entities, and (for players) the team average or total, then hover or tap any game for the details.')]));
  ...
  // Curated lines (named on the Line Analysis tab) plus "All Lines" = the whole
  // team (every point). Each carries the pointKeys computeLineStats() needs.
  function allLineEntities() {
    const tournaments = getTournaments();
    const lines = (loadLinesData().lines || []).map(l => ({ name: tsLineName(l, tournaments), pointKeys: l.pointKeys, isAll: false }));
    const allKeys = [];
    games.forEach((g, gi) => (g.points || []).forEach(p => allKeys.push(pointKey(gi, p.number))));
    return [{ name: 'All Lines', pointKeys: allKeys, isAll: true }, ...lines];
  }

  function populateStatSelect() {
    statSelect.innerHTML = '';
    const groups = new Map();
    catalogFor(mode).forEach(s => {
      if (!groups.has(s.group)) { const og = el('optgroup', { label: s.group }, []); groups.set(s.group, og); statSelect.appendChild(og); }
      groups.get(s.group).appendChild(el('option', { value: s.key }, [document.createTextNode(s.label)]));
    });
    statSelect.value = statKey[mode];
  }

  // The entity picker is the same multi-select widget either way; its items are
  // the roster in player mode and the lines in line mode.
  function buildEntitySelector() {
    entitySlot.innerHTML = '';
    if (mode === 'player') {
      entitySlot.appendChild(buildPlayerSelector(sel => { selectedPlayers = sel; render(); }, { maxPlayers: 7, roleLabel: 'Players', initialSelected: selectedPlayers }));
    } else {
      lineEntities = allLineEntities();
      entitySlot.appendChild(buildPlayerSelector(sel => { selectedLineNames = sel; render(); }, { maxPlayers: 7, roleLabel: 'Lines', items: lineEntities.map(e => e.name), preserveOrder: true, initialSelected: selectedLineNames }));
    }
  }
  ...
  function onModeChange() {
    ...
    avgChip.classList.toggle('ts-hidden', !playerMode);
    totalChip.classList.toggle('ts-hidden', !playerMode);
    render();
  }
  ...
  statSelect.addEventListener('change', () => { statKey[mode] = statSelect.value; render(); });
  controlsRow.appendChild(el('label', { class: 'ts-field' }, [el('span', { class: 'ts-field-label' }, [document.createTextNode('Stat')]), statSelect]));

  // Entity multi-select (players or lines), swapped in on mode change.
  const entitySlot = el('span', { class: 'ts-entity-slot' }, []);
  controlsRow.appendChild(entitySlot);

  // Team average / total toggles (player mode only).
  ...
  const totalNote = el('p', { class: 'ts-total-note' }, [document.createTextNode('Team total is only shown for counting stats — for rates and ratings, “Team average” is the whole-team value.')]);
  totalNote.style.display = 'none';
  section.appendChild(totalNote);
  const lineNote = el('p', { class: 'ts-total-note' }, [document.createTextNode('Only “All Lines” (the whole team) is available until you name some lines on the Line Analysis tab.')]);
  ...
  // Export / copy the chart as a PNG, captioned with the current stat (title +
  // y-axis label) and the selected entities (subtitle).
  const exportRow = el('div', { class: 'ts-export-row' }, []);
  const exportBtn = el('button', { class: 'pill-btn', type: 'button' }, [document.createTextNode('Export .png')]);
  const copyBtn = el('button', { class: 'pill-btn', type: 'button' }, [document.createTextNode('Copy .png')]);
  exportBtn.addEventListener('click', () => {
    exportChartCanvas(blob => {
      if (!blob) { alert('Add something to the chart first, then export.'); return; }
      const url = URL.createObjectURL(blob);
  ...
  copyBtn.addEventListener('click', () => {
    ...
    // ClipboardItem accepts a Promise<Blob>, which keeps the copy inside the
    // click's user-activation window even though rendering the PNG is async.
    const blobPromise = new Promise(resolve => exportChartCanvas(resolve));
  ...
  // Memoised per-game EDGE rows (games/box scores don't change during a section's
  // life) so switching between advanced stats or resizing doesn't recompute them.
  const advGameCache = new Map();
  function advRowsFor(gi) {
    if (!advGameCache.has(gi)) {
      const m = new Map();
      computeAdvancedStats([gi]).forEach(r => m.set(r.player, r));
      advGameCache.set(gi, m);
    }
    return advGameCache.get(gi);
  }
  ...
  function fmtVal(v, seriesKind, stat) {
    if (stat.kind === 'rate') return Math.round(v) + '%';
    const dec = stat.dec != null ? stat.dec : (seriesKind === 'avg' ? 1 : 0);
  ...
  function draw(series, isPct, stat) {
    ...
    const narrow = W < 480;
    const mL = 40, mR = 14, mT = 14;
    const plotW = W - mL - mR;
    const slot = plotW / Math.max(1, n);
    const xFor = i => mL + slot * (i + 0.5);

    const groups = tournamentGroups();
    const hasTournLabels = groups.some(g => g.label);

    // The bottom margin holds a win/loss dot per game, the opponent's name
    ...
    const mB = NAME_GAP + nameBand + TOURN_ROW + 6;
    const plotH = narrow ? 190 : 225;
    const H = mT + plotH + mB;
    ...
    tsAxisTicks(vmin, vmax).forEach(t => {
      ...
      lbl.textContent = isPct ? t + '%' : String(t);
      svg.appendChild(lbl);
    });

    // Tournament separators (dashed vertical lines between tournament groups).
    groups.forEach((g, gi) => {
      if (gi > 0) {
    ...
    // X axis: the opponent's name per game, tilted below the axis and coloured
    // green for a win, red for a loss (the colour is the win/loss cue -- no dot).
    const namesTopY = mT + plotH + NAME_GAP;
    games.forEach((g, i) => {
      const x = xFor(i);
      const cls = g.result === 'W' ? 'ts-win' : g.result === 'L' ? 'ts-loss' : 'ts-tie';
      const nm = svgEl('text', {
        x: x, y: namesTopY, class: 'ts-gname ' + cls,
        'text-anchor': 'end', transform: `rotate(-${NAME_ANGLE} ${x} ${namesTopY})`,
      });
      nm.textContent = tsTrunc(g.opponent || ('Game ' + (i + 1)), NAME_MAX);
      svg.appendChild(nm);
    });

    // Tournament labels, centred under each group below the tilted names.
    if (hasTournLabels) {
      groups.forEach(g => {
        if (!g.label) return;
        const cx = mL + slot * (g.start + (g.end - g.start + 1) / 2);
        const spanChars = Math.floor((slot * (g.end - g.start + 1)) / 5.4);
        const t = svgEl('text', { x: cx, y: H - 5, class: 'ts-tourn', 'text-anchor': 'middle' });
    ...
    const hi = svgEl('g', { class: 'ts-hi' });
    svg.appendChild(hi);
    const overlay = svgEl('rect', { class: 'ts-overlay', x: mL, y: mT, width: plotW, height: plotH + 22, fill: 'transparent', style: 'cursor:crosshair;' });
  ...
  function render() {
    ...
    const isPct = stat.kind === 'rate';
    const canTotal = stat.kind === 'count';
    totalChip.classList.toggle('disabled', !canTotal);
    totalCb.disabled = !canTotal;
    totalNote.style.display = (mode === 'player' && !canTotal) ? 'block' : 'none';
    lineNote.style.display = (mode === 'line' && lineEntities.length <= 1) ? 'block' : 'none';
    ...
    const series = [];
    if (mode === 'player') {
      // Per-game player -> row: box score, or an EDGE row for an advanced stat.
      const dataByGame = games.map((g, gi) => {
        if (stat.source === 'adv') return advRowsFor(gi);
        const m = new Map();
      ...
      if (showTotal && canTotal) {
        const values = dataByGame.map(m => {
          ...
          if (!rows.length) return null;
          let sum = 0;
          rows.forEach(r => { const v = stat.get(r); if (v != null && !Number.isNaN(v)) sum += v; });
          return sum;
        });
    ...
    // Legend (series, then the win/loss colour key).
    legend.innerHTML = '';
    series.forEach(s => {
    ...
    if (!series.length) {
      ...
      tip.style.display = 'none';
      const msg = mode === 'line'
        ? 'Pick one or more lines (or All Lines) above to plot a stat over the season.'
        : 'Pick players above, or turn on Team average / Team total, to plot a stat over the season.';
      chartWrap.appendChild(el('p', { class: 'pitch-caption ts-empty' }, [document.createTextNode(msg)]));
      return;
    }
    [...chartWrap.querySelectorAll('.ts-empty')].forEach(e => e.remove());
    draw(series, isPct, stat);
  }

  // The stat (y-axis quantity) + the selected entities, for captioning exports.
  function currentSelectionSummary() {
    const stat = currentStat();
    let names;
    if (mode === 'player') {
      names = selectedPlayers.slice();
      if (showAvg) names.push('Team average');
      if (showTotal && stat.kind === 'count') names.push('Team total');
      return { modeLabel: 'By player', names, statLabel: stat.label };
    }
    return { modeLabel: 'By line', names: selectedLineNames.slice(), statLabel: stat.label };
  }
  ...
  // marks via CSS classes/variables, which a bare serialize would drop, so the
  // clone gets every computed paint/text property inlined first.
  function exportChartCanvas(cb) {
    const svg = chartWrap.querySelector('.ts-svg');
    if (!svg) { cb(null); return; }
    const vb = svg.viewBox.baseVal;
    const svgW = vb.width, svgH = vb.height;
    if (!svgW || !svgH) { cb(null); return; }

    const clone = svg.cloneNode(true);
    const srcEls = svg.querySelectorAll('*');
    const cloneEls = clone.querySelectorAll('*');
    const paintProps = ['fill', 'stroke', 'stroke-width', 'stroke-dasharray', 'stroke-linejoin', 'stroke-linecap', 'opacity', 'font-size', 'font-family', 'font-weight', 'text-anchor'];
    srcEls.forEach((sEl, i) => {
      const cs = getComputedStyle(sEl);
      let st = '';
      paintProps.forEach(p => { const v = cs.getPropertyValue(p); if (v) st += p + ':' + v + ';'; });
      cloneEls[i].setAttribute('style', st);
    });
    // Drop the interactive-only layers so they don't paint into the image.
    clone.querySelectorAll('.ts-overlay, .ts-crosshair, .ts-hi').forEach(e => e.remove());
...
const TOURS = {
  ...
  'line-analysis': [
    ...
    { sel: '.line-mgmt', title: 'Your lines', body: 'The lines set up for this team in the main report — pick any of them to compare.' },
  ...
  'time-series': [
    ...
    { sel: '.ts-controls', title: 'By player or by line', body: 'Switch between comparing players and comparing lines (drawn from Line Analysis, plus “All Lines” for the whole team). Pick who to plot on the right.' },
    { sel: '.ts-stat-select', title: 'Pick a stat', body: 'Counting stats, rates, and the EDGE advanced metrics for players; scoring efficiency and more for lines — the whole chart redraws for your pick.' },
    { sel: '.ts-chart-wrap', title: 'The chart', body: 'Each point is one game. The opponent name below is green for a win and red for a loss — hover or tap any game for the exact numbers.' },
  ],
  'advanced-stats': [
